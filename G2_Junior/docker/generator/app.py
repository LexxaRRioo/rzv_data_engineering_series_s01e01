...
def initialize(tables, tech_fields, cur):
    ...
    metadata = {table_name: table_schema["columns"] for table_name, table_schema in tables.items()}
    for table_name, columns in metadata.items():
        columns_list = [f"{name} {col_config['type']}" for name, col_config in columns.items()]
        columns_list.extend([f"{name} {col_config}" for name, col_config in tech_fields.items()])
        ...
        for _ in range(insert_rows_per_tick):
            ...
            for column_name, col_config in columns.items():
                if "params" in col_config:
                    row[column_name] = getattr(fake, col_config["faker"])(**col_config["params"])
                else:
                    row[column_name] = getattr(fake, col_config["faker"])()
            for column_name in tech_fields:
                if column_name == 'created_at':
                    row[column_name] = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S%z')
...
def insert_data(data, cur):
    for table_name, rows in data.items():
        for row in rows:
            ...
            values = ", ".join([f"'{value}'" if not isinstance(value, int) or not isinstance(value, float) else str(value) for value in row.values()])
...
def update_data(tables, update_not_older_than_sec, cur):
    ...
    for table_name, table_config in tables.items():
        ...
        for id in rows:
            ...
            col_names = fake.random_elements(elements=column_names, length=fake.random_int(min=1, max=len(column_names)), unique=True)
            update_clauses = []
            for col_name in col_names:
                col_config = table_config["columns"][col_name]                
                
                if "params" in col_config:
                    new_value = getattr(fake, col_config["faker"])(**col_config["params"])
                else:
                    new_value = getattr(fake, col_config["faker"])()
                    
                new_value = f"'{new_value}'" if not isinstance(new_value, int) or not isinstance(new_value, float) else str(new_value)
            ...
            cur.execute(sql)
            logger.debug(sql)
        logger.info(f'Updated {len(rows)} row(s) in {table_name} table: {updated_columns}.')
