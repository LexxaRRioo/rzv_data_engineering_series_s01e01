...
def _prepare_update_sql(df:pd.DataFrame, table) -> str:
    ...
    cols = copy.deepcopy(pk_cols_without_scd2)
        ...
        for col in cols:
            value = f"'{row[col]}'" \
                if not isinstance(row[col], int) \
                    and not isinstance(row[col], float) \
                else str(row[col])
            values.append(value)
...
@dag(default_args=default_args, description='ETL pipeline to load data from staging layer to dds layer', schedule_interval=None, catchup=False, max_active_runs=1)
def load_dds_data():
    ...
    def extract(table:str, **context) -> str:
        ...
        filepath = get_filepath(dag_id, table, "extract", execution_date, "/tmp/airflow_staging", "csv")
    ...
    def transform(filepath, table, **context) -> list[str]:
        ...
        if max_loaded_dttm:
            ...
            df_u_dataset = df_u_dataset[pd.to_datetime(df_u_dataset[increment_col]) <= max_loaded_dttm]
        ...
        execution_date = context["dag_run"].execution_date
        dag_id = context["dag"].dag_id
        filepath_i = get_filepath(dag_id, table, "transform_i", execution_date, "/tmp/airflow_staging", "csv")
        
        df_i.to_csv(filepath_i, sep=';', header=True, index=False, mode="w", encoding="utf-8", errors="strict")
        logging.info(f"Data is transformed (insert) [{df_i.shape[0]} rows]: ready to load from stg.{table} table in {filepath}")
...
and {scd2_columns[1][0]} = '9999-12-31 23:59:59';"""
            ...
            if df_u_changed_rows.shape[0] > 0:
                df_u = pd.merge(df_u_dataset, df_u_changed_rows, how='inner', on=pk_cols_without_scd2) # filter, save only updated rows
                df_u[tech_load_column[0]] = datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
                ...
                filepath_u = get_filepath(dag_id, table, "transform_u", execution_date, "/tmp/airflow_staging", "csv")
    ...
    def load(filepaths: list, table:str) -> None:
        ...
        if filepaths[1]:
            df_u = pd.read_csv(filepaths[1], sep=';', header=0, index_col=None, encoding="utf-8")
        ...
        # Insert part
        if filepaths[0]:
            df_i = pd.read_csv(filepaths[0], sep=';', header=0, index_col=None, encoding="utf-8")
